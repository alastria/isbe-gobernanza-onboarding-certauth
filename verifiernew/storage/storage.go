...
func (s *Storage) SaveWalletAuthenticationResponse(id string, cred map[string]any) error {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	credSubject := jpath.GetMap(cred, "credentialSubject")
	fmt.Printf("%#v\n", credSubject)
	mandateeEmail := jpath.GetString(cred, "credentialSubject.mandate.mandatee.email")
...
func (s *Storage) getPrivateClaimsFromScopes(ctx context.Context, userID, clientID string, scopes []string) (claims map[string]any, err error) {
	for _, scope := range scopes {
		...
		case LEARCredentialScope:
			...
			if user != nil {
				claims = appendClaim(claims, CustomClaim, user.Credential)
			}
		}
	}
	return claims, nil
...
func (s *Storage) setUserinfo(ctx context.Context, userInfo *oidc.UserInfo, userID, clientID string, scopes []string) (err error) {
	...
	for _, scope := range scopes {
		...
		case LEARCredentialScope:
			...
			userInfo.AppendClaims(CustomClaim, user.Credential)
